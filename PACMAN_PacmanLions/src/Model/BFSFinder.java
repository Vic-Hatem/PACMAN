...
public class BFSFinder {
    ...
    public BFSFinder(Ghost pb){
        this.mx = pb.pos_currx/30;
        this.my = pb.pos_curry/30;
        //init BFS map
        //creating a adjencency matrix
        map = new int[pb.pos_currx][pb.pos_curry];
        
        for(int ii=0;ii<this.mx;ii++){
            for(int jj=0;jj<this.my;jj++){
                if(pb.mazee[jj][ii]>0 && pb.mazee[jj][ii]<26){
                    map[jj][ii] = 1;
                }else{
                    map[jj][ii] = 0;
                }
